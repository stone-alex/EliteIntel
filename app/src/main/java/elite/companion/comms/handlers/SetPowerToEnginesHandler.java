...
public class SetPowerToEnginesHandler implements CommandHandler {
    ...
    @Override public void handle(JsonObject params, String responseText) {
        ...
        try {
            ...
            stepOne.addProperty("action", GameCommandMapping.GameCommand.INCREASE_ENGINES_POWER.getGameBinding());
            voiceCommandHandler.handleGrokResponse(stepOne);
            Thread.sleep(DELAY);

            JsonObject stepTwo = new JsonObject();
            stepTwo.addProperty("type", "command");
            stepTwo.addProperty("action", GameCommandMapping.GameCommand.INCREASE_SYSTEMS_POWER.getGameBinding());
            voiceCommandHandler.handleGrokResponse(stepTwo);
            Thread.sleep(DELAY);

            JsonObject stepThree = new JsonObject();
            stepThree.addProperty("type", "command");
            stepThree.addProperty("action", GameCommandMapping.GameCommand.INCREASE_ENGINES_POWER.getGameBinding());
            voiceCommandHandler.handleGrokResponse(stepThree);
            Thread.sleep(DELAY);

            JsonObject stepFour = new JsonObject();
            stepFour.addProperty("type", "command");
            stepFour.addProperty("action", GameCommandMapping.GameCommand.INCREASE_SYSTEMS_POWER.getGameBinding());
            voiceCommandHandler.handleGrokResponse(stepFour);
            Thread.sleep(DELAY);

            JsonObject stepFive = new JsonObject();
            stepFive.addProperty("type", "command");
            stepFive.addProperty("action", GameCommandMapping.GameCommand.INCREASE_ENGINES_POWER.getGameBinding());
