...
public class AiActionsMap {
    ...
    private Map<String, String> buildActionsMap() {
        ...
        map.put("check key bindings, missing key bindings, unbound keys", KEY_BINDINGS_ANALYSIS.getAction());
        map.put("bio scan progress, biology scan progress, organics scanned, how many samples", BIO_SAMPLE_IN_STAR_SYSTEM.getAction());
        map.put("exobiology samples, biology samples, organics at location, what organisms", EXOBIOLOGY_SAMPLES.getAction());
        map.put("stellar objects, planets in system, bodies in system, what planets", QUERY_STELLAR_OBJETS.getAction());
        map.put("signals in system, FSS signals, what signals, unidentified signals", QUERY_STELLAR_SIGNALS.getAction());
        map.put("geo signals, geological signals, volcanic signals", QUERY_GEO_SIGNALS.getAction());
        map.put("stations in system, what stations, nearby stations", QUERY_STATIONS.getAction());
        map.put("fleet carriers in system, carriers in system, how many carriers", QUERY_CARRIERS.getAction());
        map.put("system security, faction control, who controls, power struggle", SYSTEM_SECURITY_ANALYSIS.getAction());
        map.put("trade profile, trade settings, trading parameters", TRADE_PROFILE_ANALYSIS.getAction());
        map.put("distance to stellar object, how far to body, distance to planet {key:X}", DISTANCE_TO_BODY.getAction());
        ...
        map.put("plotted route, route analysis, current route, navigation route", PLOTTED_ROUTE_ANALYSIS.getAction());
        map.put("carrier route, carrier navigation, carrier jump route", CARRIER_ROUTE_ANALYSIS.getAction());
        map.put("trade route, trading route, current trade plan", TRADE_ROUTE_ANALYSIS.getAction());
        map.put("outfitting, ship upgrades, modules available, what modules at station", LOCAL_OUTFITTING.getAction());
        map.put("shipyard, ships for sale, what ships at station", LOCAL_SHIPYARD.getAction());
        map.put("cargo hold, what are we carrying, cargo contents, commodities on board", CARGO_HOLD_CONTENTS.getAction());
        map.put("carrier destination, where is carrier going, carrier next jump", CARRIER_DESTINATION.getAction());
        map.put("carrier tritium, carrier fuel, how much tritium, tritium supply", CARRIER_TRITIUM_SUPPLY.getAction());
        map.put("carrier status, carrier finances, carrier balance, carrier overview", CARRIER_STATUS.getAction());
        map.put("carrier ETA, when does carrier arrive, how long until carrier", CARRIER_ETA.getAction());
        map.put("distance to carrier, how far is carrier, where is our carrier", DISTANCE_TO_CARRIER.getAction());
        map.put("pirate mission, kill count, how many kills, massacre mission progress", PIRATE_MISSION_PROGRESS.getAction());
        map.put("player profile, our ranks, combat rank, trade rank, exploration rank", PLAYER_PROFILE_ANALYSIS.getAction());
        ...
        map.put("station details, station services, what services here, what does station offer", STATION_DETAILS.getAction());
        map.put("app capabilities, what can you do, your features, list capabilities", APP_CAPABILITIES.getAction());
        map.put("AI designation, what is your name, who are you", AI_DESIGNATION.getAction());
        map.put("bounties, total bounties, bounty collected, how much in bounties", TOTAL_BOUNTIES.getAction());
        map.put("distance to bubble, how far from bubble, how far from inhabited space", DISTANCE_TO_BUBBLE.getAction());
        map.put("distance to last bio sample, how far to sample, how far to last organism", DISTANCE_TO_LAST_BIO_SAMPLE.getAction());
        map.put("current time, what time is it, time on earth", TIME_IN_ZONE.getAction());
        map.put("planet biome, biome analysis, what biome {key:X}", PLANET_BIOME_ANALYSIS.getAction());
        map.put("reminder, what was the reminder, destination reminder", REMINDER.getAction());
        map.put("local stations, stations and settlements, outposts in system", ANALYZE_LOCAL_STATIONS.getAction());
        map.put("active missions, current missions, mission log, what missions", ANALYZE_MISSIONS.getAction());
