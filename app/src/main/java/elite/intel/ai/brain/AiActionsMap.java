...
public class AiActionsMap {
    ...
    public Map<String, String> actionMap(boolean isDryRun) {
        ...
        if (status.isInMainShip() || isDryRun) {
            ...
            map.put("target fsd {key:fsd}, target engines {key:drive}, target drive {key:drive}, target thrusters {key:drive}, target Power Distributor {key:power distributor}, target power plant {key:powerplant}, target powerplant {key:powerplant}, target life support {key:life support}", TARGET_SUB_SYSTEM.getAction());
            ...
            map.put("deploy shield cell, use shield cell, activate shield cell, shield cell bank, deploy power cell, fire shield cell, recharge shield, recharge shields", DEPLOY_SHIELD_CELL.getAction());
        ...
        map.put("scan the system, open fss, full scan, system scan, discovery scan, FSS, full spectrum scan, scan system, filtered spectrum scan", OPEN_FSS_AND_SCAN.getAction());
