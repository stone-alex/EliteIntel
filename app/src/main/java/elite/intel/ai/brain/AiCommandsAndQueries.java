...
public class AiCommandsAndQueries {
    ...
    private final Map<String, String> commandMap = Map.<String, String>ofEntries(
            ...
            Map.entry(" find hunting grounds, find where we can hunt pirates within X light years " + KEY_X, FIND_HUNTING_GROUNDS.getAction()),  // Fixed typo
            Map.entry(" find human tech broker, find human tech broker within X light years " + KEY_X, FIND_HUMAN_TECHNOLOGY_BROKER.getAction()),
            Map.entry(" find guardian tech broker, find guardian broker within X light years " + KEY_X, FIND_GUARDIAN_TECHNOLOGY_BROKER.getAction()),
            Map.entry(" find planet with brain trees that contains material X " + KEY_X_WITH_DISTANCE, FIND_BRAIN_TREES.getAction()),
            Map.entry(" find star system where we can mine tritium, find carrier fuel mining site " + KEY_X_WITH_DISTANCE, FIND_FLEET_CARRIER_FUEL_MINING_SITE.getAction()),
            Map.entry(" find star system where we can mine material X, find where we can mine silver " + KEY_X_WITH_DISTANCE, FIND_MINING_SITE.getAction()),
            Map.entry(" find market where we can buy commodity X, where can we buy X, find star system where we can buy tea within X ly " + KEY_X_WITH_DISTANCE, FIND_COMMODITY.getAction()),
            Map.entry(" set voice to X, change voice to X " + KEY_X, SET_AI_VOICE.getAction()),
            Map.entry(" set home system, this will be our new home ", SET_HOME_SYSTEM.getAction()),
            Map.entry(" change personality to X " + KEY_X, SET_PERSONALITY.getAction()),
            Map.entry(" change profile to X " + KEY_X, SET_PROFILE.getAction()),
            Map.entry(" turn on radio, turn off radio " + KEY_STATE, SET_RADIO_TRANSMISSION_MODE.getAction()),
            Map.entry(" set streaming mode on/off " + KEY_STATE, SET_STREAMING_MODE.getAction()),
            Map.entry(" navigate to coordinates lat/lon " + KEY_LAT_LON, NAVIGATE_TO_TARGET.getAction()),
            Map.entry(" cancel navigation ", NAVIGATION_ON_OFF.getAction()),
            Map.entry(" discovery announcements on/off " + KEY_STATE, DISCOVERY_ON_OFF.getAction()),
            Map.entry(" mining announcements on/off " + KEY_STATE, MINING_ON_OFF.getAction()),
            Map.entry(" route announcements on/off " + KEY_STATE, ROUTE_ON_OFF.getAction()),
            Map.entry(" set speed plus X " + KEY_X, INCREASE_SPEED_BY.getAction()),  // Fixed to assumed correct action
            Map.entry(" set speed minus X " + KEY_X, DECREASE_SPEED_BY.getAction()),
            Map.entry(" change trade profile, set starting budget to X " + KEY_X, CHANGE_TRADE_PROFILE_SET_STARTING_BUDGET.getAction()),
            Map.entry(" change trade profile, set max number of stops to X " + KEY_X, CHANGE_TRADE_PROFILE_SET_MAX_NUMBER_OF_STOPS.getAction()),
            Map.entry(" change trade profile, set max distance from entry to X " + KEY_X, CHANGE_TRADE_PROFILE_SET_MAX_DISTANCE_FROM_ENTRY.getAction()),
            Map.entry(" change trade profile, allow prohibited cargo on/off "+KEY_STATE, CHANGE_TRADE_PROFILE_SET_ALLOW_PROHIBITED_CARGO.getAction()),
            Map.entry(" change trade profile, allow planetary port on/off "+KEY_STATE, CHANGE_TRADE_PROFILE_SET_ALLOW_PLANETARY_PORT.getAction()),
            Map.entry(" change trade profile, allow permit systems on/off "+KEY_STATE, CHANGE_TRADE_PROFILE_SET_ALLOW_PERMIT_SYSTEMS.getAction()),
            Map.entry(" change trade profile, allow strongholds on/off "+KEY_STATE, CHANGE_TRADE_PROFILE_SET_ALLOW_STRONGHOLDS.getAction()),
            Map.entry(" drive assist on/off "+KEY_STATE, DRIVE_ASSIST.getAction()),
    ...
    private final Map<String, String> queryMap = Map.ofEntries(
            ...
            Map.entry(" queries about system security, traffic, casualties, local controlling factions and major powers, who is in charge, local government  ", SYSTEM_SECURITY_ANALYSIS.getAction()),
