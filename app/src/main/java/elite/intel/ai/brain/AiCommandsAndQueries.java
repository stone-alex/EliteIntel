...
public class AiCommandsAndQueries {
    ...
    private final static String KEY_LAT_LON = " param template: {\"lat\": X, \"lon\": Y}";
    ...
    private Map<String, String> buildCommandMap() {
        ...
        commandMap.put(" command set speed minus" + KEY_X, DECREASE_SPEED_BY.getAction());
...
//            commandMap.put(" command target wingman 3 ", TARGET_WINGMAN2.getAction());
        ...
        commandMap.put(" command change trade profile, set starting budget to" + KEY_X, CHANGE_TRADE_PROFILE_SET_STARTING_BUDGET.getAction());
        commandMap.put(" command change trade profile, set max number of stops to" + KEY_X, CHANGE_TRADE_PROFILE_SET_MAX_NUMBER_OF_STOPS.getAction());
        commandMap.put(" command change trade profile, set max distance from entry to" + KEY_X, CHANGE_TRADE_PROFILE_SET_MAX_DISTANCE_FROM_ENTRY.getAction());
    ...
    private Map<String, String> buildQueryMap() {
        ...
        queryMap.put(" query distance to stellar object" + KEY_X, DISTANCE_TO_BODY.getAction());
        queryMap.put(" query analyze last scan ", LAST_SCAN_ANALYSIS.getAction());
        queryMap.put(" query inventory for" + KEY_X, MATERIALS_INVENTORY.getAction());
