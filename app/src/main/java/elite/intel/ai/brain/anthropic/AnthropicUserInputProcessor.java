 ...
 */
public class AnthropicUserInputProcessor extends CommandEndPoint implements AiCommandInterface {

    private static final Logger log = LogManager.getLogger(AnthropicUserInputProcessor.class);
    private static final AnthropicUserInputProcessor INSTANCE = new AnthropicUserInputProcessor();

    private ExecutorService executor;
    private final AtomicBoolean running = new AtomicBoolean(false);
    private final SystemSession systemSession = SystemSession.getInstance();

    private AnthropicUserInputProcessor() {
        // singleton
    }

    public static AnthropicUserInputProcessor getInstance() {
        return INSTANCE;
    }
    ...
    public void onSensorDataEvent(SensorDataEvent event) {
        ...
        executor.submit(() -> {
            JsonObject response = AnthropicCommandEndPoint.getInstance().processAiPrompt(messages, 0.1f);
    ...
    private void processVoiceCommand(String userInput, float confidence) {
        ...
        JsonObject response = AnthropicCommandEndPoint.getInstance().processAiPrompt(request, 0.1f);
        if (response == null) {
