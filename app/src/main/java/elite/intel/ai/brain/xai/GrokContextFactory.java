...
public class GrokContextFactory implements AiContextFactory {
    ...
    private void inputClassificationClause(StringBuilder sb) {
        sb.append("Classify input as one of:\n" +
                "    - 'command': Triggers an app action or keyboard event (DO SOMETHING). Use for inputs starting with verbs like 'set', 'get', 'switch', 'calculate', 'drop', 'retract', 'deploy', 'find', 'locate', 'activate' (e.g., 'deploy landing gear', 'add mining target'). \n"+
